...
import msgpack
import pandas as pd
from drqa.model_RN_selfattn2 import DocReaderModel
...
# system
parser.add_argument('--log_file', default='output_selfattn2.log',
...
def main():
    ...
    for epoch in range(epoch_0, epoch_0 + args.epochs):
        ...
        if not args.save_last_only or epoch == epoch_0 + args.epochs - 1:
            model_file = os.path.join(model_dir, 'checkpoint_selfattn2_noconcat.pt')
            model.save(model_file, epoch)
            if f1 > best_val_score:
                best_val_score = f1
                copyfile(
                    model_file,
                    os.path.join(model_dir, 'best_model_selfattn2_noconcat.pt'))
