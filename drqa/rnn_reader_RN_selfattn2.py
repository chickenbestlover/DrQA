...
from . import layers_RN_selfattn2 as layers
...
class RnnDocReader(nn.Module):
    ...
    def __init__(self, opt, padding_idx=0, embedding=None, normalize_emb=False,embedding_order=True):
        ...
        '''
        #self.doc_attn = layers.doc_LinearSeqAttn(input_size=doc_hidden_size,output_size=self.num_object)

        '''
        m-dim attention2 + dot-product
        '''
        self.doc_attn = layers.doc_LinearSeqAttn2(input_size=doc_hidden_size, hidden_size=doc_hidden_size, output_size=self.num_object)
