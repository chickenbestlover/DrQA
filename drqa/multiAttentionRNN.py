...
class AttentionRNN(nn.Module):
    def __init__(self, opt, doc_input_size, question_input_size,ratio=2):
        ...
        self.dropout_rate =opt['dropout_emb']
    ...
    def forward(self, x1, x1_mask, x2, x2_mask):
        ...
        for i in range(self.num_layers):
            ...
            x1 = nn.functional.dropout(x1, p=self.dropout_rate,
                                               training=self.training)
            #print(i,' x1',x1.size())
            #print(i,' x2',x2.size())
            x2 = self.question_rnns[i](x2,x2_mask)
            x2 = nn.functional.dropout(x2, p=self.dropout_rate,
                                               training=self.training)
            ...
            matched_x2_hiddens = nn.functional.dropout(matched_x2_hiddens, p=self.opt['dropout_emb'],
                                               training=self.training)
            matched_x2_hiddens = self.doc_convs[i](matched_x2_hiddens)
            matched_x2_hiddens = nn.functional.dropout(matched_x2_hiddens, p=self.opt['dropout_emb'],
                                           training=self.training)
            matched_x1_hiddens = self.doc_attns[i](x2,x1,x1_mask)
            matched_x1_hiddens = nn.functional.dropout(matched_x1_hiddens, p=self.opt['dropout_emb'],
                                               training=self.training)
